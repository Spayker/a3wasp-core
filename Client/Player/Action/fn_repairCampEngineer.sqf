...
if ((missionNamespace getVariable "WF_C_CAMPS_REPAIR_PRICE") > 0) then {
	...
	if ((Call WFCL_FNC_GetPlayerFunds) < (missionNamespace getVariable "WF_C_CAMPS_REPAIR_PRICE")) exitWith {
	    [format [parseText(localize "STR_WF_Repair_Camp_NoFunds"), (missionNamespace getVariable "WF_C_CAMPS_REPAIR_PRICE") - (Call WFCL_FNC_GetPlayerFunds)]] spawn WFCL_fnc_handleMessage
	};
...
[(parseText(localize "STR_WF_Repair_Camp_IsBeingRepaired"))] spawn WFCL_fnc_handleMessage;
//--- Begin the repair.
...
if (!(alive _vehicle) || (_vehicle distance _camp > _range)) exitWith {
    [(parseText(localize "STR_WF_Repair_TooFar"))] spawn WFCL_fnc_handleMessage
};

if (!isObjectHidden _camp) exitWith {
	[(parseText(localize "STR_WF_Repair_Camp_IsAlive"))] spawn WFCL_fnc_handleMessage;
